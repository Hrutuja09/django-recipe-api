...
class PrivateIngredientApiTests(TestCase):
    ...
    def test_ingredient_list_limited_to_user(self):
        ...
        other_user = create_user(email='other@example.com', password='test123') # unauthenticated user
        Ingredient.objects.create(user=other_user, name="Salt")
        ingredient = Ingredient.objects.create(user=self.user, name="Pepper") # authenticated user

        res = self.client.get(INGREDIENTS_URL)

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res.data),1) #only ingredient to be returned
    ...
    def test_delete_ingredient(self):
        ...
        self.assertFalse(ingredients.exists()) # no ingredients
